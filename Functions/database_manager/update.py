    ...
    if con.execute(f"SELECT 1 FROM DÉJÀ_VU WHERE Code={x}").fetchone():
        ...
        if list(rows[0])[1]=='Serie':
            ...
            if nbr_saison_vu ==0:
                ...
                del  rows [8:]
                cur.execute("INSERT INTO A_VOIR(Code, Type, Titre, Image , Etat, Date, Durée, Saison) VALUES(?, ?, ?, ?, ?, ?, ?, ?)", rows)
                ...
                cur.execute("INSERT INTO SAISONS_NON_VU (Code, Type, Titre, Image , Etat, Date, Durée, Saison,Saison_vu, Saison_non_vu) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", rows)
                con.execute(f"UPDATE DÉJÀ_VU SET Saison_vu = {nbr_saison_vu} WHERE Code ={x}")
                con.execute(f"UPDATE DÉJÀ_VU SET Saison_non_vu = {nbr_saison_non_vu} WHERE Code ={x}")
            else:
                con.execute(f"UPDATE SAISONS_NON_VU SET Saison_vu = {nbr_saison_vu} WHERE Code ={x}")
    ...
    for i in range (0,taille):
        ...
        if type=='Serie':
            ...
            if int(z[3])>0 and nbr_s==0:
                ...
                dataa.append(z[3])
                print(dataa)
                cur.execute("INSERT INTO SAISONS_NON_VU (Code, Type, Titre, Image , Etat, Date, Durée, Saison,Saison_vu, Saison_non_vu) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", dataa)
