...
def add_a_voir(data, con, cur):
    try:
        ...
        clear_terminal()
        print(
            f"{data[1]}: '{data[2]}' {data[5]}, {Texts['successfully_added_a_voir']} \n\n******************\n\n"
        )
    except:
        clear_terminal()
        print(
            f"{data[1]}: '{data[2]}' {data[5]}, {Texts['already_added_a_voir']} \n\n******************\n\n"
        )
...
def add_deja_vu(data, con, cur):
    try:
        ...
        if data[1] == "Serie":
            print(
                f"{data[1]}: '{data[2]}' {data[5]}, a {data[7]} {Texts['info_nbr_saisons']}\n"
            )
            try:
        ...
        try:
            ...
            pass
        cur.execute(
            "INSERT INTO DÉJÀ_VU(Code, Type, Titre, Image , Etat, Date, Durée, Saison,Saison_vu, Saison_non_vu) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            data,
        )
        ...
        clear_terminal()
        print(
            f"{data[1]}: '{data[2]}' {data[5]}, {Texts['successfully_added_deja_vu']}\n\n******************\n\n"
        )
    except:
        clear_terminal()
        print(
            f"{data[1]}: '{data[2]}' {data[5]}, {Texts['already_added_deja_vu']} '\n\n******************\n\n"
        )
