...
def show_lists(rows, x):
    ...
    if x == 0:
        for i in range(0, len(rows)):
            print(
                f".   {list(rows[i])[0]}, {list(rows[i])[1]}: '{list(rows[i])[2]}' {list(rows[i])[4]} {Texts['on']} {list(rows[i])[5]}\n"
            )
    else:
        for i in range(0, len(rows)):
            print(
                f".   {list(rows[i])[0]}, {list(rows[i])[1]}: '{list(rows[i])[2]}' {list(rows[i])[4]} {Texts['on']} {list(rows[i])[5]}, {Texts['saisons_to_watch1']} {list(rows[i])[9]} {Texts['saisons_to_watch2']}\n"
            )
