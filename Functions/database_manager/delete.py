...
def delete_from_a_voir(x, cur):
    ...
    info = get_data(x)
    print(
        f"{info[1]}: '{info[2]}' {info[5]}, {Texts['successfully_deleted_a_voir']}\n\n******************\n\n"
    )
...
def delete_from_deja_vu(x, cur):
    ...
    info = get_data(x)
    print(
        f"{info[1]}: '{info[2]}' {info[5]}, {Texts['successfully_deleted_deja_vu']}\n\n******************\n\n"
    )  # os.system("clear")
...
def delete(x, con, cur):
    if con.execute(f"SELECT 1 FROM A_VOIR WHERE Code={x}").fetchone():
        ...
        info = get_data(x)
        print(
            f"{info[1]}: '{info[2]}' {info[5]}, {Texts['not_found']}\n\n******************\n\n"
        )
    con.commit()
