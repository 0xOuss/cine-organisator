...
from Functions.database_manager.delete import (
    delete,
    delete_from_a_voir,
    delete_from_deja_vu,
)
...
from Functions.database_manager.select import (
    check_existence_a_voir,
    check_existence_deja_vu,
    select_a_voir,
    select_deja_vu,
    select_element_a_voir,
    select_element_deja_vu,
    select_saisons_non_vues,
)
...
def main_menu(con, cur):
    ...
    while cho1 != 0:
        ...
        if cho1 == 1:
            while cho1 == 1:
                cho = input_err_handling(
                    Texts["menu_cho1_1"], Texts["try_again"], [0, 1, 2, 3]
                )
                clear_terminal()
                try:
                    if cho == 1 or cho == 2:
            ...
            if check_existence_a_voir(x, con=con):
                ...
                cho = input_err_handling(
                    Texts["menu_cho1_3_a_voir"], Texts["try_again"], [0, 1, 2, 3]
                )
                clear_terminal()
                if cho == 1:
                ...
                cho = input_err_handling(
                    Texts["menu_cho1_3_deja_vu"], Texts["try_again"], [0, 1, 2, 3, 4]
                )
                clear_terminal()
                if cho == 1:
            ...
            while cho1 == 4:
                cho = input_err_handling(
                    Texts["menu_cho1_4"], Texts["try_again"], [0, 1, 2, 3, 4]
                )
                clear_terminal()
                if cho == 1:
            ...
            while cho1 == 5:
                cho = input_err_handling(
                    Texts["menu_cho1_5"], Texts["try_again"], [0, 1, 2, 3]
                )
                clear_terminal()
                if cho == 1:
                    x = input(Texts["enter_name_code"])
